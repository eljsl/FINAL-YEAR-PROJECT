...
%Record data from the active hydrophone in array position 4. Save data set
%as Test1H4.
Test1H4=Record_new_data();

%Import the previously recorded data and the newly recorded data, cross
%correlate each set and find the relative distances from each hydrophone to
%the source
[d1,d2,d3,d4]=Get_distances();

%Define the circles with radii d1,d2,d3,d4 from the respective hydrophone
xp1=d1*cos(ang);
yp1=d1*sin(ang);
xp2=d2*cos(ang);
yp2=d2*sin(ang);
...
%Combine all the possible intersection coordinates into an array
...
%Run function to calculate the source location using the above array
[mx,my]=Calculate_location(xypos);
